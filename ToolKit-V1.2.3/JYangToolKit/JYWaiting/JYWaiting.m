...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"JYWaiting needs to be accessed on the main thread.");

CGFloat const JYProgressMaxOffset = 1000000.f;
...
// Deprecated
@property (copy, nullable) JYWaitingCompletionBlock completionBlock;
...
@implementation JYWaiting

#pragma mark - Class methods

+ (instancetype)showWaitAddedTo:(UIView *)view animated:(BOOL)animated {
    JYWaiting *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideWaitForView:(UIView *)view animated:(BOOL)animated {
    JYWaiting *hud = [self WaitForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (JYWaiting *)WaitForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (JYWaiting *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = JYWaitingAnimationFade;
    _mode = JYWaitingModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(JYWaitingAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct
    if (type == JYWaitingAnimationZoom) {
        type = animatingIn ? JYWaitingAnimationZoomIn : JYWaitingAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == JYWaitingAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == JYWaitingAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == JYWaitingAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == JYWaitingAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    id<JYWaitingDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    JYBackgroundView *backgroundView = [[JYBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = JYWaitingBackgroundStyleSolidColor;
    ...
    JYBackgroundView *bezelView = [JYBackgroundView new];
    ...
    UIButton *button = [JYWaitingRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[JYRoundProgressView class]];
    
    JYWaitingMode mode = self.mode;
    if (mode == JYWaitingModeIndeterminate) {
    ...
    else if (mode == JYWaitingModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[JYBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == JYWaitingModeDeterminate || mode == JYWaitingModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[JYRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == JYWaitingModeAnnularDeterminate) {
            [(JYRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == JYWaitingModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == JYWaitingModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    } else if ([indicator isKindOfClass:[JYRoundProgressView class]]) {
        ((JYRoundProgressView *)indicator).progressTintColor = color;
        ((JYRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
    } else if ([indicator isKindOfClass:[JYBarProgressView class]]) {
        ((JYBarProgressView *)indicator).progressColor = color;
        ((JYBarProgressView *)indicator).lineColor = color;
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
    JYBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(JYWaitingMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == JYWaitingModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation JYRoundProgressView
...
@implementation JYBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = JYWaitingBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
            ...
            _style = JYWaitingBackgroundStyleSolidColor;
...
- (void)setStyle:(JYWaitingBackgroundStyle)style {
    if (style == JYWaitingBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = JYWaitingBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    JYWaitingBackgroundStyle style = self.style;
    if (style == JYWaitingBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == JYWaitingBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
        self.backgroundColor = self.color;
#else
...
@implementation JYWaiting (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [JYWaiting allHUDsForView:view];
    for (JYWaiting *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable JYWaitingCompletionBlock)completion {
...
- (BOOL)dimBackground {
    JYBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == JYWaitingBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = JYWaitingBackgroundStyleSolidColor;
...
@implementation JYWaitingRoundedButton
